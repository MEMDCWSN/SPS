...
namespace specnets
{
  ...
  private:
    ...
    SpecSet * m_inputSpectraMS2_normalized; //! The set of normalized input spectra to be aligned (non-scored MS/MS spectra, used for cosine matching)

    ExecTagFilterPairs * m_tag_filter;//structure for tag-based filtering pairs

    const bool ownInput; //! Does this object "own" the input data pointers (and hence have to free them)
    bool ownInputSpecsNormalized; //! Does this object "own" the input data pointers (and hence have to free them)
    bool ownInputTagFilter; //! Does this object "own" the input data pointers (and hence have to free them)
    ...
    const bool ownOutput; //! Does this object "own" the output data pointers (and hence have to free them)

    void normalizeInputSpectra();
    void loadTagFilter();
  };
