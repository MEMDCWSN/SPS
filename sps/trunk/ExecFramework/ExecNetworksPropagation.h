/*
 * ExecNetworksPropagation.h
 *
 *  Created on: Oct 20, 2015
 *      Author: isna
 */

#ifndef EXECNETWORKSPROPAGATION_H_
#define EXECNETWORKSPROPAGATION_H_
...
class ExecNetworksPropagation : public ExecBase
  {
  public:
	//! \name CONSTRUCTORS
    //@{
     ...
     */
	ExecNetworksPropagation(const ParameterList & inputParams);
    ...
    //! \name DESTRUCTOR
    //@{
    virtual ~ExecNetworksPropagation(void);
    //@}
  ...
  private:
      ...
      float	m_seedFDR;
      float m_edgeFDR;
      vector<NetworkedPSM*> *	m_propagationResults;

      float selectSeeds(PeptideSpectrumMatchSet & selectedSeeds, float seedFDR, float minScoreforSeeds, bool removeMultiplexed);

      void propagateAnnotations(PeptideSpectrumMatchSet & seeds, float fragTol, float minEdgeScore=0);

      void depthFirstPropagation(vector<vector<pair<int, float> > > & neighbors,
								  int from,
								  int to,
      ...
      int selectIdentificationsByFDR(float specifiedFDR);

  };

} /* namespace specnets */

#endif /* EXECNETWORKSPROPAGATION_H_ */
