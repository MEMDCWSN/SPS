#ifndef __ExecSpectraExtractionTable_H__
#define __ExecSpectraExtractionTable_H__
...
namespace specnets
{
   ...
   */
  class ExecSpectraExtractionTable : public ExecBase
  {
  public:
    //! \name CONSTRUCTORS
    //@{
    /*! \brief The exemplar constructor.
     ...
     @sa clone()
     */
    ExecSpectraExtractionTable(void);
     ...
     */
    ExecSpectraExtractionTable(const ParameterList & inputParams);
    ...
    //! \name DESTRUCTOR
    //@{
    virtual ~ExecSpectraExtractionTable(void);
    //@}
  ...
  private:
    //Load existing library, then add new spectra, and save
    void add_to_library();
    
    //Load existing spectra from table object
    int load_new_spectra();
    ...
    std::string library_grid_root_dir;
    std::string library_user;

    std::string input_table_filename;
    
    std::vector<std::vector<std::string> > annotation_lines; 
    std::vector<std::string> annotation_headers;
    std::map<std::string, unsigned int> annotation_headers_map;
    
    std::string path_to_spectra;
