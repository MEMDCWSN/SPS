...
namespace specnets
{
  ...
  ExecSpectralLibrarySLGFTraining::ExecSpectralLibrarySLGFTraining(const ParameterList & inputParams){
    ...
    stringSplit(inputParams.getValue("trainpsmnames"), m_training_psm_file_names);      //D where are these files? a PSM list
  ...
  bool ExecSpectralLibrarySLGFTraining::invoke(void){
    ...
    m_distilled_spectral_lib.setPeakTolerance(0.5, false);      //D default value of mass tolerance is 0.5?
