...
namespace specnets {
	...
	static float	flanking_mass_limit = 6000;
...
/*	ExecTagFilterPairs::ExecTagFilterPairs(const ParameterList & inputParams,
	...
	// -------------------------------------------------------------------------
	bool ExecTagFilterPairs::prepare(SpecSet * prmSpectra, string tags_file, int numTags, bool matchingFlankingMass) {

		ownInput= false;
		m_prmSpectra= prmSpectra;
		m_maxTagSize= numTags;//m_params.getValueInt("MAX_TAG_SIZE", 50);
		...
		//construct index
		if( matchingFlankingMass ) indexTags( 0 );
		...
		while( fgets(buf, 512, fp) != NULL ){
			...
			if( strstr(buf, ">>") ){
				...
				while( fgets(buf, 512, fp) != NULL ){//read lines
					...
					int seqID = aaIndex(tseq[0])*400 + aaIndex(tseq[1])*20 + aaIndex(tseq[2]);      //D get the order of these amino acid
					tag_T tag = {(int)round(pm), seqID, (int)round(sm+AAJumps::massH2O), sc};       //D round prefix masses
					st.push_back( tag );
				}
				...
				for(vector<tag_T>::iterator itr=st.begin(); itr!=st.end(); itr++) {
					for(vector<tag_T>::iterator ktr=itr+1; ktr!=st.end(); ktr++) {
						if( itr->seqKey == ktr->seqKey ){
							if( abs(itr->ntMass - ktr->ntMass ) < iso_err_reward ){     //D 1.5
								st.erase(ktr);
								ktr--;
							}
						}
				...
				sort(st.begin(), st.end(), tag_score_comparator);       //D sort by scores decreasingly
				int size = 0;
				for(vector<tag_T>::iterator itr=st.begin(); itr!=st.end(); itr++){
	...
	void ExecTagFilterPairs::indexTags(int startSpec) {
		...
		for(int i=startSpec; i<setSize; i++){
			for(vector<tag_T>::iterator itr=(*m_spec_tags)[i].begin(); itr!=(*m_spec_tags)[i].end(); itr++){
				(*nterm_prefix_fmMap)[itr->seqKey][itr->ntMass].push_back(i);       //D store the indices of spectra containing the tag
