...
namespace specnets
{
    ...
    class SpectralLibrary : public SpecSet
    {
        public:
            ...
            virtual int search(Spectrum &query_spec, PeptideSpectrumMatchSet &output_psms, float parentmass_tolerance, float ion_tolerance, int top_hits, int analog_search = 0, float score_threshold = 0.0, int library_search_quality = 1000, int minimum_shared_peaks=1000, bool forceExact=false);
            ...
            //DW
            int search_target_decoy_SLGFNew2(vector< list <pair<string, float> > > target_tag_lib, vector< list <pair<string, float> > > decoy_tag_lib, list <pair<string, float> > query_tags,
            ...
            //DW
            vector< list <pair<string, float> > > extract_search_tags(SpecSet, const char*);
    ...
    class SpectralLibraryGFSearch : public SpectralLibrary
    {
        ...
        private:
            ...
            vector<vector<pair<float, float> > > histograms;        //D set of histograms but only one deletion_prob
