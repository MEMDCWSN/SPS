...
namespace specnets
{
  ...
  //---------------------------------------------------------------------------
  bool SpectrumPairSet::saveToTSVFile(const std::string & filename)
  {
	FILE *txtfp = fopen(filename.c_str(), "w");
	if (txtfp == 0)
	{
	  ERROR_MSG("Can not open: " << filename);
	  return false;
	...
	for(vector<SpectrumPair>::iterator spr=thePairs.begin(); spr!=thePairs.end(); spr++){
		fprintf(txtfp, "%d\t%d\t%.3f\t%.3f\t%.4f\t%.4f\t%d\n",
							spr->spec1+1, spr->spec2+1, spr->shift1, spr->shift2,
								spr->score1, spr->score2, spr->specC+1);
	}
	fclose(txtfp);
      ...
      unsigned int max_unique_mass_number)
  {
    ...
    int compIndex= 0;
    for(vector<vector<int> >::iterator ctr=specs_in_comp.begin(); ctr!=specs_in_comp.end(); ctr++){
    	if( ctr->size() != 0 ){
    		for(vector<int>::iterator spec=ctr->begin(); spec!=ctr->end(); spec++){
    			refer_to_comp[*spec]= compIndex;
    		}
    		compIndex++;
    	}
    }
    ...
    for(vector<SpectrumPair>::iterator spr=thePairs.begin(); spr!=thePairs.end(); spr++){
    	spr->specC= refer_to_comp[spr->spec1];
    	if( refer_to_comp[spr->spec1] == -1 || refer_to_comp[spr->spec1] != refer_to_comp[spr->spec2] )
    		DEBUG_MSG("ERORR:: " << spr->spec1 << "X" << spr->spec2);
    }
  ...
  bool SpectrumPairSet::filter_by_score(float minScore)
  {
	...
	for (int i = 0; i < thePairs.size(); i++) {
	  ...
	  if (min(thePairs[i].score1, thePairs[i].score2) < minScore) {
