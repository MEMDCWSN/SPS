...
namespace specnets
{
  ...
  void SpecSet::setPeakTolerance(float tolerance, bool applyPPM)
  {
    for (int i = 0; i < size(); i++)
    {
      ...
      {
        specs[i].setPeakTolerance(tolerance);       //D set peaktolerance in dalton
      }
    }
  }
  ...
  unsigned int SpecSet::LoadSpecSet_mgf(const char *filename)
  {
    ...
    if (blr.Load(filename) <= 0 or blr.size() < 3)
      return 0; // A valid file must have at least 3 lines: BEGIN_IONS, END_IONS and one mass/intensity peak
    cout << "blr.size() = " << blr.size() << endl;
    DEBUG_MSG("blr.size() = " << blr.size() << endl);

    unsigned int lineIdx, specIdx, peakIdx;

    //D Counts number of spectra and number of peaks per spectrum
    list<int> peaksPerSpec;
    int first;
    int numPeaks = 0; // Counts number of peaks in the spectrum
    for (lineIdx = 0; lineIdx < blr.size(); lineIdx++)
    ...
    cout << "mgf spectra, peaksPerSpec.size() = " << peaksPerSpec.size() << endl;
    DEBUG_MSG("mgf spectra, peaksPerSpec.size() = " << peaksPerSpec.size() << endl);
    ...
    for (specIdx = 0; specIdx < specs.size(); specIdx++)
    {
          ...
          and strncmp("END IONS", blr.getline(lineIdx), 8) != 0)
      {
        ...
        if (line[0] >= 48 and line[0] <= 57)
        {
          ...
          {
            cerr << "Error loading " << filename
                << " - could not parse peak intensity on line " << lineIdx << " " << line
                << " of specIdx " << specIdx
                << "!\n";
            resize(0);
            return 0;
        ...
        {
          //D DEBUG_MSG("&line[7] = " << &line[7]);
          //D DEBUG_MSG("atof(&line[7]) = " << atof(&line[7]));
          specs[specIdx].parentCharge = (short)atof(&line[7]);
          //D DEBUG_MSG("specs[specIdx].parentCharge = " << specs[specIdx].parentCharge);
        }
        ...
        if (strncmp("PRECURSORINTENSITY=", line, sizeof("PRECURSORINTENSITY=") - 1) == 0)
        {
          specs[specIdx].precursor_intensity = atof(line + sizeof("PRECURSORINTENSITY=") - 1);
        }
        if (strncmp("TOLU=", line, sizeof("TOLU=") - 1) == 0)
        {
          specs[specIdx].TOLU = line + sizeof("TOLU=") - 1;
  ...
  short SpecSet::SaveSpecSet_mgf(const char* filename, bool outputScans)
  {
    ...
    for (int i = 0; i < specs.size(); i++)
    {
      ...
      if (specs[i].retention_time > 0)
        fprintf(output, "RTINSECONDS=%f\n", specs[i].retention_time);
  ...
  bool SpecSet::loadPklBin_2(FILE* fp, int numSpecs, char subversion)
  {
    ...
    for (unsigned int i = 0; i < numSpecs; i++)
    {
      ...
      else if (fragTypes[i] == (short)Spectrum::FragType_NONE)
      {
        specs[i].msFragType = Spectrum::FragType_NONE;
      }
      else
      {
  ...
  void SpecSet::saveSNRRankedTags(const std::string & filename, int tagLen, int gap, int tagNum, float fragTol, bool isPRMSpec)
    {
  //	FILE *fp = fopen(filename.c_str(), "w");
  //	if (fp == 0) {
  //	  ERROR_MSG("Can not open: " << filename);
  ...
  //
  //	  list<Tag> tags;
  //	  ExtractSNRRankedTags(specs[i], tags, fragTol, tagLen, gap, tagNum);
  //
  //	  for (list<Tag>::iterator itr = tags.begin(); itr != tags.end(); itr++) {
  //		string seq;
  //                string rseq;
  //		for (int c = 0; c < itr->sequence.size(); c++) {
  //		  seq  += jumps.aaLetters[itr->sequence[c]];
  //                  rseq += jumps.aaLetters[itr->sequence[itr->sequence.size() - c - 1]];
  //		}
  //		fprintf(fp, "%d\t%.3f\t%s\t%.3f\t%.3f", i+1, itr->flankingPrefix-ionOffset, seq.c_str());
  //		float first_offset = itr->flankingPrefix - AAJumps::massHion -AAJumps::massH2O;
  //                float flanking_offset = specs[i].parentMass-(itr->peak_positions.back());
  //                fprintf(fp, "\t_+%.3f.%s_+%.3f",flanking_offset,rseq.c_str(),first_offset);
  //                fprintf(fp, "\n");
  //	  }
  ////	  fprintf(fp, "\n");
  //	}
  //	fclose(fp);
    ...
    void SpecSet::saveSNRRankedTags2(const std::string & filename, int tagLen, int gap, int tagNum, float fragTol, bool isPRMSpec)
    {
  	FILE *fp = fopen(filename.c_str(), "w");
  	if (fp == 0) {
  	  ERROR_MSG("Can not open: " << filename);
  ...
  //	  fprintf(fp, ">>%d$\t%.2f\t%d\n", i+1, specs[i].parentMass, specs[i].parentCharge);
  	  ...
  	  list<Tag> tags;
  	  //D ExtractTags(specs[i], tags, fragTol, tagLen, gap, tagNum, 0.0, false);
  	  ExtractDenovoTags(specs[i], tags, fragTol, tagLen, tagNum, false);

            if (tags.size() > 0) {
                  float precursor_mass = (float) ((specs[i].parentMass) + (specs[i].parentCharge - 1) * specnets::AAJumps::massHion) / specs[i].parentCharge;

                  fprintf(fp, "%s\t%d\t%.2f\t%d\t%d", specs[i].fileName.c_str(), specs[i].scan, precursor_mass, specs[i].parentCharge, tags.size());
                  ...
                  for (list<Tag>::iterator itr = tags.begin(); itr != tags.end(); itr++) {
                    ...
                    fprintf(fp, "\t%.2f\t%.2f\t%s",itr->flankingPrefix-ionOffset, itr->score, seq.c_str());
                  }
                  fprintf(fp, "\n");
            }
