...
//
                     ...
                     bool generateLabels)
  {
    ...
    if (glbMasses.empty())
    {
      glbMasses.resize(AAcount);        //D AAcount = 20
      glbLetters.resize(AAcount);
      glbMods.resize(AAcount);
      for (aaIndex = 0; aaIndex < AAcount; aaIndex++)
      {
        glbMasses[aaIndex] = AAmasses[aaIndex];     //D an array of amino acids masses
        glbMods[aaIndex].resize(0);     //D a vector of double vectors
      }
      for (aaIndex = 0; aaIndex < AAcount; aaIndex++)
        glbLetters[aaIndex] = AAletters[aaIndex];       //D an array of amino acids letters
    }
...
//
                         ...
                         bool generateLabels)
  {
    ...
    if (maxJumpSize == 0)
    {
      ...
      if (m_generateLabels)
      {
        jumpLabels.resize(1);       //D first - label &   second - mass
        jumpLabels[0].first = "";
        ...
        for (modIndex = 0; modIndex < refMods[aaIndex].size(); modIndex++)
        {
          if (m_generateLabels)
          {
            jumpLabels[idxUse].first = getModString(refLetters[aaIndex],
                                                    refMods[aaIndex][modIndex]);        //D idxUse = aaIndex?
...
// Test function
  ...
  bool AAJumps::getAllAArefs(vector<pair<char, float> > & returnAAs)
  {
    for (int i = 0; i < refLetters.size(); i++)
    {
      pair<char, float> newPair = make_pair<char, float>(refLetters[i],
                                                         refMasses[i]);
                             ...
                             const bool addZeroMass) const
  {
    ...
    for (unsigned int i = start; i < size - start; i++)
    {
      ...
      if ('[' == sequence_ptr[i])
      {
        ...
        //D masses[spec_index] = f + total; // added
        //D spec_index++; // added
        total += f;
      ...
      else if (sequence_ptr[i] == '(')
      {
        ...
        { // check if we have a N-term modification inside another modification
          ...
          if (j >= float_i && i - j >= 2)
          {
            ...
            // get the string
            strncpy(float_seq2, &sequence_ptr[j + 1], i - j - 1);       //D not care amino acids in the inside ()?
...
// -------------------------------------------------------------------------
  ...
  /**D AAJumps::getPRMandSRMMasses
   * Calculates SRM masses for input annotation in SpecNets format *.SEQ[-17]UENCE.*
   * @param sequence: Annotation of sequence in SpecNets format (i.e. brackets around modifications)
   * @param prm_masses: 	Vector of prefix masses to be modified.
   * @param srm_masses: 	Vector of suffix masses to be modified.
   * @param pre_offset: 	Prefix offset value.
   * @param suf_offset: 	Suffix offset value.
   * @param peptide_mass: Summed mass of all amino acids. NOT THE SAME AS PRECURSOR MASS.
   * @param addZeroMass: 	True if we need the first mass is for prefix mass.
   *
   */
  bool AAJumps::getPRMandSRMMasses(const string &sequence,
                                   vector<float> &prm_masses,
                                   ...
                                   bool addZeroMass) const
  {
    bool sequenceOK = getPRMMasses(sequence, prm_masses, 0.0, (vector<string> *)0, addZeroMass);

    int prm_length = prm_masses.size();

    peptide_mass = prm_masses[prm_length - 1];

    srm_masses.resize(prm_length);      //D updated
    int i;
    int mass_i = 0;

    for (i = prm_length - 1; i >= 0; i--)       //D updated
    {
      srm_masses[mass_i] = prm_masses[prm_length - 1] - prm_masses[i]; //total mass of peptide minus masses starting from end.
      mass_i++;
    }

    for (i = 0; i < prm_length; i++)
        prm_masses[i] = prm_masses[i] + pre_offset;

    for (i = 0; i < prm_length; i++)
        srm_masses[i] = srm_masses[i] + suf_offset;
