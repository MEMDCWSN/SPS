...
namespace specnets
{
  ...
  // -------------------------------------------------------------------------
  /**
   * Sets the ppm tolerance of each peak        //D a value for each peak can be different
   */
  ...
  {
    map<float, float> peakTolMap;       //D temp
  ...
  void Spectrum::rankFilterPeaks(int maxRank, float windowRadius)
  {
    ...
    for (int i = 0; i < numPeaks; i++)
    {
      ...
      {
        rankedPeaks.push_back(TwoValues<float>(peakList[j][1], (float)j));      //D (intensity, index)
        --j;
      }
      j = i + 1;
  ...
  void Spectrum::selectTopK(unsigned int topK, Spectrum *putHere)
  {
    ...
    if (topK < oldPeakList->size())
    {
      ...
      sort(sortedInts.begin(), sortedInts.end());       //D sort increasingly

      updatedPeakList.resize(topK);
      for (peakIdx = 1; peakIdx <= topK; peakIdx++)
        updatedPeakList[peakIdx - 1] =
            (*oldPeakList)[(unsigned int)sortedInts[sortedInts.size() - peakIdx][1]];       //D [1] is for peakIdx
                             ...
                             bool preprocess) const
  {
    ...
    FindMatchPeaksAll2_efficient(spec1,
                       ...
                       peakTol,
                       idxMatched1_other,
                       idxMatched2_other);      //D the match is always better than the zero_match
    ...
    //D idea: select the index pair of highest cosine, then remove (marked as 1), then repeat
    for (int i = (int)peakMatches.size() - 1; i >= 0; i--)
    {
      if (peakUsed1[peakMatches[i].i] == 0 and peakUsed2[peakMatches[i].j] == 0)
      {
        cosine += peakMatches[i].score;     //D numerator
        peakUsed1[peakMatches[i].i] = 1;
        peakUsed2[peakMatches[i].j] = 1;
        score1 += spec1[peakMatches[i].i][1];       //D sum of matched intensities in spec1
        score2 += spec2[peakMatches[i].j][1];       //D sum of matched intensities in spec2
    ...
    if (not projectedCosine)
      ...
      cosine = cosine / (sqrt(matchedPeaksNorm1) * sqrt(matchedPeaksNorm2));        //D where is sqrt
   ...
   if (projectedCosine) {
       ...
       for (int i = (int)peakMatches.size() - 1; i >= 0; i--)
       {
         if (peakUsed1[peakMatches[i].i] == 0 and peakUsed2[peakMatches[i].j] == 0)
         {
           peakUsed1[peakMatches[i].i] = 1;
           peakUsed2[peakMatches[i].j] = 1;
           cosine2 += spec1[peakMatches[i].i][1] * (spec2[peakMatches[i].j][1]/sqrt(matchedPeaksNorm2));
         }
       }
       //D cosine = min(cosine1,cosine2);
       //D cosine = max(cosine1,cosine2);
       cosine = cosine1;
   }
  ...
  //      cosine = cosine;
  //    else
  //      cosine = cosine / min(matchedPeaksNorm1, matchedPeaksNorm2);

   return cosine;
  }


    //D for scoring between SLGF's modification spectra
...
//       spec2[j][1] = sqrt(spec2[j][1]);
   ...
   peakUsed1.resize(spec1.size());
   peakUsed2.resize(spec2.size());
   //D float pmDiff = spec1.parentMass - spec2.parentMass;
   //D consider the charge issue
   float pmDiff = spec1.parentMZ*spec2.parentCharge - (spec2.parentCharge-1)*AAJumps::massHion - spec2.parentMass;      //D query_charge <- reference_charge
                            ...
                            bool preprocess) const
  {
   ...
   for (int i = (int)peakMatches.size() - 1; i >= 0; i--)
   {
     if (peakUsed1[peakMatches[i].i] == 0 and peakUsed2[peakMatches[i].j] == 0)
     {
       cosine += peakMatches[i].score;
       for (int j = i; j >= 0; j--)
            if ((peakMatches[i].i-peakMatches[j].i) * (peakMatches[i].j-peakMatches[j].j) <= 0)     //D eliminate matches crossing
            {
                peakUsed1[peakMatches[j].i] = 1;
                peakUsed2[peakMatches[j].j] = 1;
            }
       score1 += spec1[peakMatches[i].i][1];
       score2 += spec2[peakMatches[i].j][1];
       matchedPeaksNorm1 += spec1[peakMatches[i].i][1]
