...
short BufferedLineReader::Load(const char *filename)
{
  ...
  unsigned long long numBytes = ftell(input);
  if (numBytes == 0)
  ...
  //D DEBUG_MSG("numBytes = " << numBytes << endl);

  lines = (char *)malloc(numBytes);
  lines[numBytes - 1] = '\0';
  unsigned long long readBytes = fread(lines, 1, numBytes - 1, input);
  ...
  unsigned long long numLines = 0, pivot;
  ...
  //D DEBUG_MSG("numLines = " << numLines << endl);
  cout << "sizeof(size_t) = " << sizeof(size_t) << endl;
  cout << "sizeof(lines) = " << sizeof(lines) << endl;
  ...
  for (pivot = 0; pivot < numBytes; pivot++)
  {
    if (lines[pivot] == '\n')
    {
      linesIdx[numLines++] = pivot + 1;     //D data lines starts from 1, '\0' is used to separate lines
      lines[pivot] = '\0';      //D pivots to separate lines
    }
  }
  fclose(input);
  return 1;
