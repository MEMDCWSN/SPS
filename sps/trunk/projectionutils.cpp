...
float spectrum_similarity(psmPtr psm1, string annotation1,  psmPtr psm2, string annotation2,
    ...
    //D 3rd parameter is equal to 1 - Applying SQRT
...
void preprocess_spectrum_intensities_max_intensity(Spectrum* s, float max){
    //D DEBUG_MSG("s->size() = " << s->size() << endl);
    float current_max_intesity = 0.f;
    for(int i = 0; i < s->size(); i++){
        //D DEBUG_MSG("(*s)[i][1] = " << (*s)[i][1] << endl);
        if(current_max_intesity < (*s)[i][1]) current_max_intesity = (*s)[i][1];
    }

    //D DEBUG_MSG("current_max_intesity = " << current_max_intesity << endl);
    for(int i = 0; i < s->size(); i++){
...
void extractIons(psmPtr psm, int peptideLength, MS2ScoringModel &model,
	...
	//D Normalize before extraction, so that we take into account noise
...
void extractIons(psmPtr psm, int peptideLength, MS2ScoringModel &model,
	...
	// Select target ions
	unsigned int baseIdx=0;
	for(unsigned int ionType=0; ionType<ionsToExtract.size(); ionType++) {		//D all of the types
		for(unsigned int peakIdx=0; peakIdx<psm->m_peakAnnotations.size(); peakIdx++)       //D for each peak of the spectrum
			if(psm->m_peakAnnotations[peakIdx].first and psm->m_peakAnnotations[peakIdx].first->name==ionsToExtract[ionType]){
...
float spectrum_similarity_sqrt_librarypeaks_isocombine_speed_optimization(psmPtr library,
    ...
    DEBUG_MSG("spectrum_similarity_sqrt_librarypeaks_isocombine_speed_optimization" << endl);
    ...
    normalize_extracted_ions(library->m_ion_extraction);       //D added in Jan 2018
...
float spectrum_similarity_sqrt_librarypeaks(psmPtr library,
    ...
    /*D
    for(int i = 0; i < library->m_ion_extraction.size(); i++){
        cout << library->m_ion_extraction[i] << " ";
    ...
    //D
    for(int i = 0; i < query->m_ion_extraction.size(); i++){
        cout << query->m_ion_extraction[i] << " ";
        DEBUG_MSG(query->m_ion_extraction[i] << " ");
    }
    cout << endl << "m_ion_extraction---------------------------------" << endl;
    DEBUG_MSG(endl << "m_ion_extraction---------------------------------" << endl);
    D*/
...
float spectrum_similarity_sqrt_librarypeaks_speed_optimization(psmPtr library,
    ...
    //D get the square root of the sum of the squares of intensities
    float explained_intensity = preprocess_library_ion_extraction(psm2temp, peptideLength, model, ionsToExtract, allIons);      //D psm2temp is psm of query spectrum
    ...
    for(int i = 0; i < library->m_ion_extraction.size(); i++){
        if(library->m_ion_extraction[i] < 0.0001){
            psm2temp->m_ion_extraction[i] = 0.f;        //D query is set to 0.f
        }
    }
    ...
    normalize_extracted_ions(psm2temp->m_ion_extraction);
    normalize_extracted_ions(library->m_ion_extraction);        //D added on Jan 2018

    cout << "printtt" << endl;
    for (int i = 0; i < library->m_ion_extraction.size(); i++)
        cout << library->m_ion_extraction[i] << "\t";
    cout << endl << endl;
    for (int i = 0; i < psm2temp->m_ion_extraction.size(); i++)
        cout << psm2temp->m_ion_extraction[i] << "\t";
    cout << endl;


    float cosine_val = cosine(library->m_ion_extraction, psm2temp->m_ion_extraction);       //D cosine between library and query spectra
...
float preprocess_library_ion_extraction(psmPtr library,
    ...
    //D DEBUG_MSG("BEGIN preprocess max" << endl);
    preprocess_spectrum_intensities_max_intensity(library->m_spectrum, 1000.f);
    //D DEBUG_MSG("END preprocess max" << endl);

    preprocess_spectrum_intensities(library->m_spectrum, 0, 1);

    //D DEBUG_MSG("BEGIN annotation" << endl);
    library->annotate(library->m_annotation,allIons,model,0,0,.50);
    //D DEBUG_MSG("END annotation" << endl);

    //D DEBUG_MSG("BEGIN extractIons" << endl);
    extractIons(library,peptideLength,model,ionsToExtract,library->m_ion_extraction, 0, 0);
    //D DEBUG_MSG("END extractIons" << endl);
    ...
    normalize_extracted_ions(library->m_ion_extraction);        //D just do normalize
...
float preprocess_library_ion_extraction_isocombine(psmPtr library,
    ...
    preprocess_spectrum_intensities(library->m_spectrum, 0, 1);     //D one kind of normalizing
...
vector<pair<float, float> > create_histogram(int buckets, float start_range, float end_range, vector<float> input, bool normalize){
    ...
    for(int i = 0 ; i < buckets; i++){
        ...
        histogram_cell.first = start_range + (end_range - start_range)/buckets * i + (end_range - start_range)/buckets * 0.5;       //D centers at the middle
...
void dynamicprogramming_spectrum(Spectrum &spectrum, MS2ScoringModel model, vector<string> ions_to_extract, string allIons, vector<float> ion_mean, vector<float> ion_variance, vector< pair < float, float > > insertion_probability , vector< pair < float, float > > deletion_probability, vector< pair< float, float> > &output_distribution, vector<vector<pair<float, float> > > histograms, int training_model){
    ...
    psm->annotate(psm->m_annotation,allIons,model,0,0,.45);     //D peak tolerance = 0.45?
    //D bool PeptideSpectrumMatch::annotate(const string &peptide, const string &ionNamesInclude, const MS2ScoringModel &inputIonTypes, const float prmOffset, const float srmOffset, const float peakTol
    ...
    deleteIons(psm, create_deliminated_aminoacids(psm->m_annotation).size(), model,ionsToExtract_precursor);        //D Why delete ions on ionsToExtract_precursor
    ...
    vector<float> ori_ion_mass_intensity_pair_vector = ion_mass_intensity_pair_vector;
    sqrt_vector(ion_mass_intensity_pair_vector);        //D WHY DO WE NEED TO DO SQRT?
    normalize_extracted_ions(ion_mass_intensity_pair_vector);


    dynamicprogramming_ionvector(ori_ion_mass_intensity_pair_vector,
                                 ion_mass_intensity_pair_vector,
                                  ion_mean,
                                  ion_variance,
...
void dynamicprogramming_ionvector(vector<float> ori_ion_intensities,
                                  vector<float> ion_intensities,
    ...
    float PROB_EPS = 0.0000000001;        //D no consider probabilities which are less than this value
    ...
    //D sort in descending???
    ion_mass_intensity_pair_vector = sort_descending(ion_intensities);
    ori_ion_intensities = sort_descending(ori_ion_intensities);

    for(int i = 0; i < ion_mass_intensity_pair_vector.size(); i++){
        if(ion_mass_intensity_pair_vector[i] < 0.00001){
            ion_mass_intensity_pair_vector.erase(ion_mass_intensity_pair_vector.begin()+i);     //D delete intensity < epsilon
            ori_ion_intensities.erase(ori_ion_intensities.begin()+i);
            i--;
        }
    }

    ion_mass_intensity_pair_vector.insert(ion_mass_intensity_pair_vector.begin(), 1, 0.f);      //D the first intensity = 0
    ...
    for(int intensity_idx = 0; intensity_idx < intensity_bins; intensity_idx++){        //D intensity_idx is the norm index in the paper
    ...
    //Initializing table
    set_3d_array(intensity_bins, cosine_bins, ion_size, 0, 0, 0, programming_matrix, 1.f);      //D F[0][0][0] = 1
    queue<triple> Q;
    Q.push(triple(0, 0, 0));        //D for the cell F[0][0][0]
    int enqueue_times = 1;
   ...
   //=======================================================================
    DEBUG_MSG("ion_size = " << ion_size);
    int number_bands = 8;
    float bounds[] = {7500, 12500, 17500, 22500, 27500, 45000, 90000, 1000000000};
    //D for(int ion_idx = 0; ion_idx < ion_size-1; ion_idx++){
    while (!Q.empty()){
        triple indices = Q.front();
        int intensity_idx = indices.first;
        int cosine_idx = indices.second;
        int ion_idx = indices.third;
        Q.pop();

        double ion_intensity = ion_mass_intensity_pair_vector[ion_idx+1];       //D ignore the first intensity that is equal to 0?
        //D note: all ion_intensity were normalized
        float percent_intensity = min(ion_intensity/max_intensity, 0.999999);
        //D int band_idx = min( ((int)(min(ion_intensity/max_intensity, 0.999999)/(1.0/ion_variance.size()))), ((int)(ion_variance.size() - 1)));       //D ion_variance.size() is the number of bands
        //D DEBUG_MSG("ion_variance.size() = " << ion_variance.size() << endl);
        int band_idx;
        for (band_idx = 0; band_idx < number_bands; band_idx++)
        {
            if (ori_ion_intensities[ion_idx] <= bounds[band_idx])
                break;
        }

        if(band_idx >= number_bands){
            DEBUG_MSG("ori_ion_intensities[ion_idx] = " << ori_ion_intensities[ion_idx] << endl);
            cout<<"BAND IDX ERROR"<<"\t"<<band_idx<<endl;
            DEBUG_MSG("BAND IDX ERROR"<<"\t"<<band_idx<<endl);
            break;
        }

        //D DEBUG_MSG("Progress\t"<<ion_idx<<" of "<<ion_size);


        //D for(int intensity_idx = 0; intensity_idx < intensity_bins; intensity_idx++){
        //D    for(int cosine_idx = 0; cosine_idx< cosine_bins; cosine_idx++){
        ...
        if(current_probability < PROB_EPS) continue;        //D not consider too small value probabilities


        //Deletion Probability
        float specific_deletion_probability = b_search(deletion_probability, percent_intensity);
        ...
        double new_probability_for_deletion = current_probability * specific_deletion_probability + next_same_cell_probability;     //D in case of deletion, cosine and norm (intensity) are the same
        ...
        if ((next_same_cell_probability < PROB_EPS) and (new_probability_for_deletion >= PROB_EPS))     //D consider the old and new probabilities for updating a new cell to the dynamic programming table
            if (ion_idx+1 < ion_size-1)
            {
                Q.push(triple(intensity_idx, cosine_idx, ion_idx+1));       //D deletion case
                enqueue_times++;
            }
        ...
        //DW vector< pair<double, double> > cumulative_histogram = get_cumulative_values(histograms[band_idx]);
        ...
        for(int forward_intensity_idx = intensity_idx; forward_intensity_idx < intensity_bins; forward_intensity_idx++){
            ...
            int forward_cosine_idx = forward_cosine_value*cosine_bins;      //D rounded up?
            ...
            double current_gaussian_lookup = points_in_gaussain[forward_intensity_idx - (intensity_idx)];       //D forward_intensity_idx is always greater than intensity_idx
            ...
            switch(training_model){
                ...
                case 4://Empirical without mean offset
                    ...
                    ion_variance_probability = ion_variance_integration(histograms, band_idx, previous_gaussian_lookup, current_gaussian_lookup, 0.f);      //D sum of probabilities on the interval [previous_gaussian_lookup, current_gaussian_lookup)
                    //DW ion_variance_probability = ion_variance_integration2(cumulative_histogram, previous_gaussian_lookup, current_gaussian_lookup, 0.f);
                    break;
                default:
            ...
            double specific_nondeleted_probability = (1.f - specific_deletion_probability);     //D in case of non-deletion
            ...
            if(propogating_probability != propogating_probability){     //D what???
                DEBUG_MSG("NAN at ion\t"<<ion_idx<<"\tintensity\t"<<forward_intensity_idx<<"\tcosine\t"<<forward_cosine_idx<<"\t"<<normalizing_factor);
            }

            double forward_probability = get_3d_array(intensity_bins, cosine_bins, ion_size, forward_intensity_idx, forward_cosine_idx, ion_idx+1, programming_matrix);     //D for forward values
            ...
            if ((forward_probability < PROB_EPS) and (propogating_probability >= PROB_EPS))     //D consider the old and new probabilities for updating a new cell to the dynamic programming table
                if ((forward_intensity_idx<intensity_bins) and (forward_cosine_idx<cosine_bins) and (ion_idx+1<ion_size-1))
                {
                    Q.push(triple(forward_intensity_idx, forward_cosine_idx, ion_idx+1));
                    enqueue_times++;
                }
        ...
        }       //D end for forward_intensity_idx

            //D}       //D end for cosine_idx
        //D}       //D end for intensity_idx
    }       //D end the dynamic programming calculating
    ...
    }       //D end display output

    double total_prob = 0.f;
    for(int cosine_idx = 0; cosine_idx< cosine_bins; cosine_idx++){
        total_prob += get_3d_array(intensity_bins, cosine_bins, ion_size, intensity_bins-1, cosine_idx, ion_size-1, programming_matrix);        //D note: intensity_bins-1 corresponds to the norm value of 1
    }

    if(total_prob != 0.f){
        for(int cosine_idx = 0; cosine_idx< cosine_bins; cosine_idx++){
            float cosine_bin = cosine_idx * (1.f/cosine_bins);      //D real cosine value
            ...
            output_distribution.push_back(cosine_prob_pair);        //D SLGF OUTPUT_DISTRIBUTION
            output_stream<<cosine_bin<<"\t"<<normalized_bin_prob<<endl;
        }
    }

    //D print out the dynamic programming matrix
    int non_zero = 0;
    for (int i = 0; i < dynamic_prog_table_size; i++)
        //if (programming_matrix[i] > 0.0f)
        if (programming_matrix[i] >= 0.0000000001)
            non_zero++;
    cout << non_zero << "/" << dynamic_prog_table_size << " elements are non-zeros" << endl;
    DEBUG_MSG(non_zero << "/" << dynamic_prog_table_size << " elements are non-zeros" << endl);
    cout << "enqueue_times = " << enqueue_times << endl;
    DEBUG_MSG("enqueue_times = " << enqueue_times << endl);
...
double ion_variance_integration(vector<vector<pair<float, float> > > &histograms, int bandidx, double left, double right, double mean_adjustment){
    ...
    if(left > 4.0 && right > 4.0){      //D 4 is the maximum index in histogram
        return 0.f;
    }
...
//DW
vector< pair<double, double> > get_cumulative_values(vector< pair<float, float> > histogram)
{
    vector< pair<double, double> > cumulative_histogram;
    double sum = 0.0;
    for (int i = 0; i < histogram.size(); i++)
    {
        sum = sum + histogram[i].second;
        pair<float, float> histogram_pair;
        histogram_pair.first = histogram[i].first;
        histogram_pair.second = sum;
        cumulative_histogram.push_back(histogram_pair);
    }
...
//DW
double ion_variance_integration2(vector< pair<double, double> > cumulative_histogram, double left, double right, double mean_adjustment){
    //double integration = 0.f;
    ...
    */
    ...
    int i = 0, j = cumulative_histogram.size()-1;

    int L = 0;      //D find the ending position of the interval
    int R = cumulative_histogram.size() - 1;
    int M;

    while (L <= R)
    {
        M = (L+R) / 2;
        if ((right + mean_adjustment) > cumulative_histogram[M].first)
        {
            j = M;
            L = M + 1;
        }
    ...
    L = 0;      //D find the begin position of the interval
    R = cumulative_histogram.size() - 1;

    while (L <= R)
    {
        M = (L+R) / 2;
        if ((left + mean_adjustment) <= cumulative_histogram[M].first)
        {
            i = M;
            R = M - 1;
        }
    ...
    //D get sum of probabilities on [i, j]
    if (i == 0)
        return cumulative_histogram[j].second;
    else
        return cumulative_histogram[j].second - cumulative_histogram[i-1].second;
    //return integration;
}
...
float SLGF_rescore(vector<pair < float, float> > &cosine_distribution, float cosine){
    ...
    //D all code above is meaningless
...
*/
float b_search(vector< pair<float, float> > deletion_probability, float percent_intensity)
{
    float result = 0.0f;
    int left = 0;
    int right = deletion_probability.size() - 1;
    int middle;

    while (left <= right)
    {
        middle = (left+right) / 2;
        if (percent_intensity > deletion_probability[middle].first)
        {
            result = deletion_probability[middle].second;
            left = middle + 1;
        }
        else
        {
